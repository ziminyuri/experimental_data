...
class MainWindow(Frame):
    def __init__(self, root):
        ...
        file_menu.add_command(label="Открыть изображение", command=self.button_open_image)
        file_menu.add_command(label="Открыть звуковой файл", command=self.button_open_sound)
        ...
        processing_menu.add_command(label="Деконволюция", command=self.button_deconvolution)
    ...
    def button_deconvolution(self):
        ...
        self.combobox_analysis_model_1 = ttk.Combobox(a, values=[u"Input:кардиограмма", u"Output:кардиограмма"],
                                                      height=4, width="28")
        ...
        self.combobox_analysis_model_2 = ttk.Combobox(a, values=[u"Input:кардиограмма", u"Output:кардиограмма"],
                                                      height=4, width="28")
        ...
        b1 = Button(a, text="Построить", command=lambda: self.task_build(a), width="30", height="2")
        b1.place(x=900, y=120)

        b2 = Button(a, text="Спектр", command=lambda: self.task_spectrum(a), width="30", height="2")
        b2.place(x=900, y=170)

        b3 = Button(a, text="Деконволюция", command=lambda: self.task_reverse(a), width="30", height="2")
        b3.place(x=900, y=220)

        b4 = Button(a, text="Закрыть", command=lambda: self.click_button_close(a), width="30", height="2")
    ...
    def add_button_play(self, model):
        ...
        if chart_number == "1":
            self.button_play_sound = Button(text="Воспроизведение", command=lambda: self.play(model), width="14",
                                            height="2")
